...
`settings.html` content: a settings page with sections, rows, toggle switches, chevrons, a long scroll. Every control is **inert** — clicking does nothing. Style it to look like a plausible in-app settings page.

Include some CSS that costs real compositing work — `backdrop-filter: blur()`, box shadows, a sticky header, rounded overflow clipping. This is not sabotage; it is representative of what real in-app web content looks like, and it gives the HUD something honest to react to under scroll.
...
Then scroll it, and let the HUD make the argument for you.
...
- If the HUD shows no measurable difference between tabs 1 and 2, the page is too light. Add content and compositing cost until the difference is honest and visible — but never fake it.
...
- [ ] Every control in `settings.html` is inert.
- [ ] Scrolling tab 2 produces a **visible, reproducible** change in the HUD's host-thread numbers relative to tab 1.
